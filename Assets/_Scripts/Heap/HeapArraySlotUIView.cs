...
/// <summary>Heap 내부 배열의 고정 슬롯 하나를 표시한다.</summary>
public class HeapArraySlotUIView : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("새 배열 슬롯 값이 등장할 때 시작 크기 배율입니다. 1보다 작으면 작게 시작합니다.")]
    private float _appearStartScale = 0.7f;

    [FoldoutGroup("Animation")]
    [SerializeField, Tooltip("새 배열 슬롯 값이 등장할 때 시작 투명도입니다.")]
    private float _appearStartAlpha = 0.25f;

    [FoldoutGroup("Animation")]
    [SerializeField, Tooltip("등장 중 원래 크기보다 살짝 커지는 배율입니다.")]
    private float _appearOvershootScale = 1.05f;

    [FoldoutGroup("Animation")]
    [SerializeField, MinValue(0f), Tooltip("작고 흐린 슬롯이 커지고 선명해지는 시간입니다.")]
    private float _appearGrowDuration = 0.14f;

    [FoldoutGroup("Animation")]
    [SerializeField, MinValue(0f), Tooltip("커진 슬롯이 원래 크기로 돌아오는 시간입니다.")]
    private float _appearSettleDuration = 0.14f;

    [FoldoutGroup("Animation")]
    [SerializeField, Tooltip("비교 강조 pulse에서 커지는 배율입니다.")]
    private float _pulseScale = 1.06f;
    ...
    // 작고 흐린 상태에서 살짝 커지며 나타난 뒤 원래 크기로 돌아오는 등장 연출.
    ...
    // 이미 보이는 슬롯 살짝 커졌다가 원래 크기로 돌아오는 강조 연출

...
public class DequeController : MonoBehaviour
{
    ...
    {
        if (!IsStarted || !ParseValue(_controls.ValueInput, out object value)) 
            return;
    ...
    {
        if (!IsStarted 
            || !ParseIndex(false, out int index)
            || !ParseValue(_controls.IndexValueInput, out object value)) 
            return;
    ...
    private void RefreshView(int firstIndex = 0, int changedCount = 0, int selectedIndex = -1)
    {
        ...
        int start = startIterator.Node * _deque.BlockSize + startIterator.Curr;
    ...
    private bool ParseIndex(bool inserting, out int index)
    {
        ...
        if (int.TryParse(_controls.IndexInput, out index)
            && index >= 0
            && index <= maximum) 
            return true;
